//ESP SENSÓRICA
// hace lo mismo que la otra, recibe datos de los sensores, pero no guarda los datos en la sd sino que los envia a la de gestion de datos
#include <Arduino.h>
...
void setup() {
  ...
  if(cardType == CARD_NONE){
    Serial.println("No hay SD card anexa");
    return;
  }

  // Si el archivo data.txt no existe
  // Crea un archivo en la SD y escribe los datos
  File file = SD.open("/data.txt");
  if(!file) {
    Serial.println("El archivo data.txt no existe");
    Serial.println("Creando archivo...");
    writeFile(SD, "/data.txt", "dist_recorrida, velocidad, mediciones[0], mediciones[1], mediciones[2] \r\n");
  }
  else {
